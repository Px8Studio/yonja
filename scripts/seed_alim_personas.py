...
Usage:
    python scripts/seed_alim_personas.py

    Or with database URL:
    DATABASE_URL="postgresql://..." python scripts/seed_alim_personas.py

    To persist to database:
    python scripts/seed_alim_personas.py --to-db

The script can also be run from Chainlit directly:
    - Go to demo-ui/
    - python -c "from alim_persona import PersonaProvisioner; p = PersonaProvisioner.generate_gold_standard_scenario('cotton_farmer_sabirabad'); print(p.to_dict())"
"""
...
DB_AVAILABLE = False
try:
    from alim_persona_db import save_alim_persona_to_db
...
def seed_all_personas(persist_to_db: bool = False):
    ...
    if persist_to_db:
        print("💾 Persisted to database: alim_personas table")
    print("=" * 70)
...
def print_persona_for_login(user_name: str) -> str:
    ...
    Usage:
        python scripts/seed_alim_personas.py --for-login "John Smith"
    """
