#!/usr/bin/env python
"""🌾 Yonca AI - LangGraph Checkpoint Table Setup Script.
...
async def setup_langgraph_tables():
    ...
    """
    print("═" * 60)
    print("🌾 Yonca AI - LangGraph Checkpoint Table Setup")
    print("═" * 60)
    ...
    print(f"\n📍 Target Database: {psycopg_url[:50]}...")
    ...
    try:
        ...
        print(f"\n❌ Failed to setup LangGraph tables: {e}")
        print("\n💡 Possible causes:")
        ...
        if "ProactorEventLoop" in str(e):
            print("\n⚠️  Windows Event Loop Issue Detected!")
...
async def verify_tables():
    ...
    print("\n" + "═" * 60)
    print("🔍 Verifying LangGraph Tables")
    print("═" * 60)
    ...
    try:
        ...
        if tables:
            print("\n✅ LangGraph tables found:")
            for row in tables:
                print(f"   • {row['table_name']}")
            return True
        else:
            print("\n⚠️  No LangGraph checkpoint tables found!")
            print("   Tables may be created on first agent invocation.")
            return False

    except ImportError:
        print("\n⚠️  asyncpg not available for direct verification")
...
async def main():
    ...
    if tables_exist:
        print("\n✅ LangGraph tables already exist! No action needed.")
        return

    # Try to create tables
    print("\n" + "═" * 60)
    print("🛠️  Attempting to Create LangGraph Tables")
    print("═" * 60)
    ...
    if success:
        ...
        print("\n💡 Alternative: Run the demo-ui and start a conversation.")
