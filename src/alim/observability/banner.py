# src/ALİM/observability/banner.py
...
Example usage:
    from alim.observability.banner import print_startup_banner, print_status_line
...
ALİM_ASCII = r"""
  ═══════════════════════════════════════════════════
  [ALİM] ALİM — Kənd Təsərrüfatı Köməkçisi
  ═══════════════════════════════════════════════════
"""

ALİM_ASCII_COMPACT = "[ALİM] ALİM — Kənd Təsərrüfatı Köməkçisi"
...
def print_startup_banner(
    ...
    print(_c("  [ALİM] ALİM — Kənd Təsərrüfatı Köməkçisi", Colors.BRIGHT_WHITE))
...
def print_shutdown_message() -> None:
    ...
    print(_c("  🌿 ALİM shutting down gracefully", Colors.BRIGHT_YELLOW))
    print(_c("  ═══════════════════════════════════════════", Colors.BRIGHT_YELLOW))


def print_startup_complete(service_name: str = "ALİM") -> None:
...
def print_tier_comparison() -> None:
    """Print a comparison table of all ALİM infrastructure tiers."""
    # Import here to avoid circular import
    from alim.config import INFERENCE_TIER_SPECS, InferenceTier

    print_section_header("[INFRA]  ALİM Infrastructure Matrix — All Tiers")
    print()
...
if __name__ == "__main__":
    ...
    # ALİM Infrastructure Tier
    print_infrastructure_tier(settings.inference_tier_spec)

    # Infrastructure
    print_section_header("🔌 Infrastructure")
    print_status_line(
        "PostgreSQL", "Connected", "success", "localhost:5433/ALİM — user data & sessions"
    )
    print_status_line(
    ...
    print_startup_complete("ALİM API")
