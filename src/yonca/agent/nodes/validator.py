...
from yonca.agent.state import AgentState, RuleMatch
...
SAFETY_RULES = {
    ...
    "extreme_cold": {
        "condition": lambda state: (state.get("weather") and 
                                    state["weather"].temperature_c is not None and 
                                    state["weather"].temperature_c < -5),
    ...
    "extreme_heat": {
        "condition": lambda state: (state.get("weather") and 
                                    state["weather"].temperature_c is not None and 
                                    state["weather"].temperature_c > 40),
    ...
    "heavy_rain": {
        "condition": lambda state: (state.get("weather") and 
                                    state["weather"].precipitation_mm is not None and 
                                    state["weather"].precipitation_mm > 30),
...
AGRONOMIC_VALIDATIONS = {
    ...
    "no_irrigation_in_rain": {
        "condition": lambda state: (
            state.get("intent") and state["intent"].value == "irrigation" and
            state.get("weather") and 
            state["weather"].precipitation_mm is not None and 
            state["weather"].precipitation_mm > 10
        ),
    ...
    "no_spraying_in_wind": {
        "condition": lambda state: (
            state.get("intent") and state["intent"].value == "pest_control" and
            state.get("weather") and 
            state["weather"].wind_speed_kmh is not None and 
            state["weather"].wind_speed_kmh > 20
        ),
    ...
    "no_planting_in_frost": {
        "condition": lambda state: (
            state.get("intent") and state["intent"].value == "planting" and
            state.get("weather") and 
            state["weather"].temperature_c is not None and 
            state["weather"].temperature_c < 5
        ),
