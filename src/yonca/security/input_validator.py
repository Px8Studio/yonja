...
INJECTION_PATTERNS: list[tuple[str, float, str]] = [
    ...
    (r'ignore\s+(previous|all|above|prior|earlier)\s+(instructions?|prompts?|rules?|guidelines?)', 0.5, "instruction_override"),
    (r'disregard\s+(your|the|all|my)\s+(rules?|instructions?|guidelines?|prompts?)', 0.5, "instruction_override"),
    (r'forget\s+(everything|all|what)\s+(you|i|we)\s+(told|said|know|learned)', 0.5, "instruction_override"),
    (r'override\s+(your|the|all)\s+(instructions?|programming|rules?)', 0.5, "instruction_override"),
    ...
    (r'(?:show|reveal|display|print|output|repeat|tell\s+me)\s+(?:your|the)\s+(?:system\s+)?(?:prompt|instructions?|programming)', 0.4, "prompt_extraction"),
    (r'what\s+(?:are|were|is)\s+(?:your|the)\s+(?:initial\s+)?(?:instructions?|prompt|rules?)', 0.4, "prompt_extraction"),
