...
LEAKAGE_PATTERNS: list[tuple[str, OutputSeverity, str]] = [
    ...
    (r'my\s+(?:system\s+)?instructions\s+(?:are|say|tell|include)', OutputSeverity.CRITICAL, "instruction_disclosure"),
    (r'I\s+(?:was|am)\s+(?:told|instructed|programmed)\s+to', OutputSeverity.WARNING, "programming_reference"),
    (r'my\s+(?:original|initial|system)\s+prompt', OutputSeverity.CRITICAL, "prompt_reference"),
    (r'my\s+(?:guidelines?|rules?|directives?)\s+(?:state|say|are)', OutputSeverity.WARNING, "guidelines_reference"),
...
JAILBREAK_PATTERNS: list[tuple[str, OutputSeverity, str]] = [
    ...
    (r"I'?ll\s+(?:ignore|bypass|skip)\s+(?:my|the)\s+(?:rules?|guidelines?|restrictions?)", OutputSeverity.CRITICAL, "rule_bypass"),
    (r"let\s+me\s+help\s+you\s+(?:bypass|circumvent|avoid)", OutputSeverity.CRITICAL, "bypass_assist"),
    (r"I'?m\s+(?:now\s+)?(?:in\s+)?(?:DAN|developer)\s+mode", OutputSeverity.CRITICAL, "dan_mode"),
    (r"(?:sure|ok|okay),?\s+I'?ll\s+(?:pretend|act)\s+(?:to\s+be|as)", OutputSeverity.CRITICAL, "roleplay_accept"),
    (r"without\s+(?:any|my)\s+(?:ethical|safety|moral)\s+(?:restrictions?|guidelines?)", OutputSeverity.CRITICAL, "ethics_bypass"),
    
    # Persona adoption
    (r"as\s+(?:a\s+)?DAN", OutputSeverity.CRITICAL, "dan_persona"),
    (r"speaking\s+as\s+(?:a\s+)?(?:hacker|criminal|villain)", OutputSeverity.CRITICAL, "malicious_persona"),
    
    # Explicit refusal to follow rules (ironically, also a jailbreak indicator)
    (r"I\s+(?:refuse|won'?t|cannot)\s+follow\s+(?:my|these|the)\s+(?:rules?|instructions?)", OutputSeverity.WARNING, "refusal_statement"),
]

# Harmful content patterns (non-agricultural)
HARMFUL_PATTERNS: list[tuple[str, OutputSeverity, str]] = [
    # Violence
    (r"\b(?:kill|murder|harm|hurt|attack)\s+(?:people|humans?|someone)", OutputSeverity.CRITICAL, "violence"),
    (r"\b(?:bomb|weapon|explosive)\s+(?:making|instructions?|how\s+to)", OutputSeverity.CRITICAL, "weapons"),
    
    # Self-harm (in non-plant context)
    (r"(?:suicide|self[- ]harm)(?!\s+(?:prevention|awareness|help))", OutputSeverity.CRITICAL, "self_harm"),
    
    # Illegal activities
    (r"(?:steal|hack|fraud|scam)\s+(?:money|data|accounts?)", OutputSeverity.CRITICAL, "illegal_activity"),
    
    # Discrimination
    (r"\b(?:racist|sexist|homophobic)\s+(?:joke|content|material)", OutputSeverity.CRITICAL, "discrimination"),
]
...
class OutputValidator:
    ...
    def validate(self, response: str) -> OutputValidationResult:
        ...
        for pattern, severity, issue_type in COMPILED_LEAKAGE:
            ...
            if match:
                issues.append(OutputIssue(
                    ...
                    severity=severity,
                    description=f"Potential system prompt leakage detected",
        ...
        for pattern, severity, issue_type in COMPILED_JAILBREAK:
            ...
            if match:
                issues.append(OutputIssue(
                    issue_type=issue_type,
                    severity=severity,
                    description=f"Jailbreak indicator detected",
                    matched_pattern=match.group()[:50] + "..." if len(match.group()) > 50 else match.group(),
                ))
        ...
        # Determine if safe
        has_critical = any(
            issue.severity == OutputSeverity.CRITICAL for issue in issues
        )
        has_warning = any(
            issue.severity == OutputSeverity.WARNING for issue in issues
        )
