...
class TestRule:
    ...
    def test_rule_evaluates_all_conditions(self, sample_rule):
        ...
        # Both conditions met
        assert sample_rule.evaluate({"weather": {"temperature_c": 35, "humidity_percent": 30}}) is True
        
        # Only first condition met
        assert sample_rule.evaluate({"weather": {"temperature_c": 35, "humidity_percent": 60}}) is False
        
        # Only second condition met
        assert sample_rule.evaluate({"weather": {"temperature_c": 25, "humidity_percent": 30}}) is False
        
        # Neither condition met
        assert sample_rule.evaluate({"weather": {"temperature_c": 25, "humidity_percent": 60}}) is False
...
class TestRulesEngine:
    ...
    def test_evaluate_returns_matches(self, engine):
        ...
        context = {
            "weather": {"temperature_c": 35, "humidity_percent": 30, "precipitation_mm": 0, "wind_speed_kmh": 5},
            "farm": {"region": "aran"},
            "intent": "irrigation",
        ...
        assert any("temperature" in m["rule_name"].lower() or "temperatur" in m["rule_name"].lower() for m in matches)
    ...
    def test_matches_sorted_by_priority(self, engine):
        ...
        context = {
            "weather": {"temperature_c": 35, "humidity_percent": 30, "precipitation_mm": 10, "wind_speed_kmh": 5},
            "intent": "irrigation",
        }
